...
export function useDerivTrading() {
 ...
 const placeTrades = useCallback(async () => {
    if (!derivWS.connected || !market) return;
    ...
    return new Promise((resolve) => {
        const tickHandler = async (data) => {
            if (!tickData && data.tick.event === 'tick') {  // first tick only
                ...
                tradeResults.forEach(trade =>
                    derivWS.send({ cancel_all: 1, contract_id: trade.contractId })
                        .catch(e => console.error('Cancel error:', e))
                );
                ...
                derivWS.off('tick', tickHandler);  // remove handler to avoid multiple triggers
            }
        };

        derivWS.on('tick', tickHandler);
    });
}, [market, stake, currency, tradeResults]);
  // ===================== END CHANGE =====================
